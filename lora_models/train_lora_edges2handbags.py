...
class DynamicPreprocessingDataset(torch.utils.data.Dataset):
    def __init__(self, dir_A, dir_B, transform):
        self.images_A = sorted(glob(os.path.join(dir_A, "*")))  # edges (A) images
        self.images_B = sorted(glob(os.path.join(dir_B, "*")))  # bags (B) images
    ...
    def __getitem__(self, idx):
        image_A = Image.open(self.images_A[idx]).convert("RGB")  # edges image
        image_B = Image.open(self.images_B[idx]).convert("RGB")  # bags image

        # Apply transforms
        image_A = self.transform(image_A)
        image_B = self.transform(image_B)

        return {"edges_images": image_A, "bags_images": image_B}

def create_dataloaders(data_dir, train_batch_size):
    # Define paths for train and test datasets
    train_dir_A = os.path.join(data_dir, "trainA")  # edges images
    train_dir_B = os.path.join(data_dir, "trainB")  # bags images
    test_dir_A = os.path.join(data_dir, "testA")    # edges images
    test_dir_B = os.path.join(data_dir, "testB")    # bags images
...
def compute_loss(pipeline, dataloader, accelerator):
    ...
    with torch.no_grad():
        for batch in dataloader:
            edges_images = batch["edges_images"].to(accelerator.device)
            bags_images = batch["bags_images"].to(accelerator.device)
            edges_images = edges_images.to(dtype=torch.float16)
            bags_images = bags_images.to(dtype=torch.float16)

            latents = pipeline.vae.encode(edges_images).latent_dist.sample()
            ...
            batch_size = edges_images.shape[0]

            # Get style conditioning from shoe images
            with torch.no_grad():
                style_latents = pipeline.vae.encode(bags_images).latent_dist.sample()
...
def train_lora(
    ...
    output_dir="lora-edges2handbags",
    num_train_epochs=20,
    train_batch_size=16,
    learning_rate=1e-4,
    rank=8,
    project_name="edges2handbags-lora",
    max_grad_norm=1.0,
    data_dir=None, 
):
    ...
    for epoch in range(num_train_epochs):
        ...
        for batch in train_dataloader:
            edges_images = batch["edges_images"].to(accelerator.device)  # Input (edges)
            bags_images = batch["bags_images"].to(accelerator.device)    # Target (bags)
    
            edges_images = edges_images.to(dtype=torch.float16)
            bags_images = bags_images.to(dtype=torch.float16)

            with accelerator.accumulate(pipeline.unet):
                latents_edges = pipeline.vae.encode(edges_images).latent_dist.sample()
                latents_bags = pipeline.vae.encode(bags_images).latent_dist.sample()
        
                latents_edges = latents_edges * pipeline.vae.config.scaling_factor
                latents_bags = latents_bags * pipeline.vae.config.scaling_factor

                noise = torch.randn_like(latents_edges)
                timesteps = torch.randint(
                0, pipeline.scheduler.config.num_train_timesteps,
                (latents_edges.shape[0],), device=latents_edges.device
                )
                noisy_latents_edges = pipeline.scheduler.add_noise(latents_edges, noise, timesteps)
...
if __name__ == "__main__":
    train_lora(
        ...
        rank=4,
        project_name="edges2handbags-lora",
        data_dir="/home/ubuntu/edges2handbags"
    )